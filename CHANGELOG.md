...
- Added `TopologyBuilder` class that can be used to build up a new topology subset from a source topology. The new topology shape does not have to match the original topology. A new coordset will be created based on the points that are referenced from the original topology.
