...
//---------------------------------------------------------------------------//
int 
send_recv_using_schema(const Node &sendnode, int dest, int sendtag, 
                       Node &recvnode, int src, int recvtag, MPI_Comm comm)
{
    //---------------------------------------------------------------------
    // Prepare send data
    //---------------------------------------------------------------------
    ...
    // schema will only be valid if compact and contig
    if( sendnode.is_compact() && sendnode.is_contiguous())
    {
        s_data_compact = sendnode.schema();
    }
    else
    {
        sendnode.schema().compact_to(s_data_compact);
    }
    ...
    //---------------------------------------------------------------------
    MPI_Status status;
    int mpi_error = MPI_Probe(src, recvtag, comm, &status); //<---- Does this spoil use of Sendrecv?
    ...
    //---------------------------------------------------------------------
    // Do both the send and recv using MPI_Sendrecv
    //---------------------------------------------------------------------
    mpi_error = MPI_Sendrecv(const_cast<void*>(n_msg.data_ptr()),
                             static_cast<int>(msg_data_size),
                             MPI_BYTE,
                             dest,
                             sendtag,
    ...
    //---------------------------------------------------------------------
    // Reconstitute the recv data into recvnode
    //---------------------------------------------------------------------
    uint8 *n_buff_ptr = (uint8*)n_buffer.data_ptr();
    ...
    // length of the schema is sent as a 64-bit signed int
    // NOTE: we aren't using this value  ... 
    n_msg2["schema_len"].set_external((int64*)n_buff_ptr);
    n_buff_ptr +=8;
    // wrap the schema string
    n_msg2["schema"].set_external_char8_str((char*)(n_buff_ptr));
    // create the schema
    Schema rcv_schema;
    Generator gen(n_msg2["schema"].as_char8_str());
    gen.walk(rcv_schema);

    // advance by the schema length
    n_buff_ptr += n_msg2["schema"].total_bytes_compact();
    
    // apply the schema to the data
    n_msg2["data"].set_external(rcv_schema,n_buff_ptr);
    
    // copy out to our result node
    recvnode.update(n_msg2["data"]);
