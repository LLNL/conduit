...
TEST(conduit_relay_io_silo, round_trip_spiral_multi_dom_materials)
{
    ...
    ASSERT_TRUE(blueprint::mesh::verify(save_mesh, info)) << info.to_yaml();
...
//     for(index_t domain_id = 0; domain_id < save_mesh.number_of_children(); domain_id ++)
//     {
//         Node &domain = save_mesh[domain_id];
...
//             matset["material_map"][mat_name].set(i);
//         }
...
//         }
//         Node info;
//         ASSERT_TRUE(blueprint::mesh::matset::verify(matset, info))
//             << "domain " << domain_id << ":" << info.to_yaml() << matset.to_yaml();
//     }
